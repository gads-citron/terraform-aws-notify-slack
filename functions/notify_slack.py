...
def decrypt_url(encrypted_url: str) -> str:
    ...
    try:
        ...
        raise RuntimeError("Unable to decrypt SLACK_WEBHOOK_URL") from e
    ...
    return {
        ...
        "fields": [
            {"title": "Alarm Name", "value": f"`{alarm_name}`", "short": True},
            {
                ...
                "value": f"`{message.get('AlarmDescription', '')}`",
                "short": False,
            },
            {
                "title": "Alarm reason",
                "value": f"`{message.get('NewStateReason', '')}`",
                "short": False,
            },
            {
                "title": "Old State",
                "value": f"`{message.get('OldStateValue', '')}`",
                "short": True,
            },
            {
                "title": "Current State",
                "value": f"`{message.get('NewStateValue', '')}`",
                "short": True,
            },
            {
                "title": "Link to Alarm",
...
def format_guardduty_finding(message: Dict[str, Any], region: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "fields": [
            {
                "title": "Description",
                "value": f"`{detail.get('description', '')}`",
                "short": False,
            },
            {
                "title": "Finding Type",
                "value": f"`{detail.get('type', '')}`",
                "short": False,
            },
            {
                "title": "First Seen",
                "value": f"`{service.get('eventFirstSeen', '')}`",
                "short": True,
            },
            {
                "title": "Last Seen",
                "value": f"`{service.get('eventLastSeen', '')}`",
                "short": True,
            },
            {"title": "Severity", "value": f"`{severity}`", "short": True},
            {"title": "Account ID", "value": f"`{detail.get('accountId', '')}`", "short": True},
            {
                "title": "Count",
                "value": f"`{service.get('count', '')}`",
                "short": True,
            },
            {
...
def format_default(
    ...
    if isinstance(message, dict):
        ...
        fields.append({"value": str(message), "short": False})
...
def get_slack_message_payload(
    ...
    if isinstance(message, dict) and "AlarmName" in message:
        ...
        attachment = format_guardduty_finding(message=message, region=message["region"])

    elif isinstance(message, dict) and message.get("detail-type") == "AWS Health Event":
        attachment = format_aws_health(message=message, region=message["region"])

    elif isinstance(message, dict) and ("attachments" in message or "text" in message):
...
def lambda_handler(event: Dict[str, Any], context: Dict[str, Any]) -> str:
    ...
    for response in responses:
        if response["code"] != 200:
            logging.error(
                f"Slack error: code={response['code']} info={response.get('info')} body={response.get('body')}"
            )
